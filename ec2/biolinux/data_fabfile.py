...
def local_server():
    ...
    """
    env.user = 'jingwenw'
    env.data_files = '/glob/jingwenw/ref/'
...
def _update_loc_file(ref_file, line_parts):
    ...
    if env.galaxy_base is not None:
        ...
        if not exists(tools_dir):
            ...
            put(os.path.join("/bubo/home/h15/jingwenw/private/galaxy/bcbb/ec2/biolinux/installed_files", conf_file),os.path.join(env.galaxy_base, conf_file))
...
def _index_twobit(ref_file):
    ...
    with cd(dir_name):
	    ...
	    print("faToTwoBit %s %s" % (os.path.join(os.pardir, ref_file),
                out_file))
...
def _index_bwa(ref_file):
	...
	print("mkdir %s" % dir_name)
        run("mkdir %s" % dir_name)
        with cd(dir_name):
	    print("ln -s %s" % os.path.join(os.pardir, ref_file))
            run("ln -s %s" % os.path.join(os.pardir, ref_file))
            with settings(warn_only=True):
		print("bwa index -a bwtsw %s" % local_ref)
...
def _index_maq(ref_file):
	...
	print("mkdir %s" % dir_name)
	    ...
	    print("ln -s %s" % os.path.join(os.pardir, ref_file))
            run("ln -s %s" % os.path.join(os.pardir, ref_file))
	    print("maq fasta2bfa %s %s" % (local_ref,
                binary_out))
...
def _index_eland(ref_file):
	...
	print("Num_refs:",num_refs)
        ...
        # For large reference sequences, squash fails and need them split up
            ...
            run("mkdir %s" % tmp_dir)
            run("seqretsplit -sequence %s -osdirectory2 %s -outseq ." %(ref_file, tmp_dir))
            with cd(tmp_dir):
                result = run("ls *.fasta")
                result = result.split("\n")
		result.pop()
            seq_files = [os.path.join(tmp_dir, f) for f in result]
	    run("squashGenome %s %s" % (os.path.join(dir_name), " ".join(seq_files)))
#            run("rm -rf %s" % tmp_dir)
